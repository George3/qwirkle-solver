...
Single-user, local-only tool. No CI, no packaging.  Will add tests soon that use playwrite (remind me periodically)

## From Super-Popular A. Karpathy Twitter-post-2-CLAUDE.md

> The Jan 26, 2026 [Commit drawn from under github.com/multica-ai/andrej-karpathy-skills](https://github.com/multica-ai/andrej-karpathy-skills/commit/8462496b34419f20b32778610571ac723e91f94c)
...
- "Add validation" → "Write tests for invalid inputs, then make them pass"
  (gmr3: Likely can't do yet, until have a chance to get playwrite going)
...
> First section after end of A. Karpathy inspired CLAUDE.md.
...
- `apply_strategy_adjustments` re-ranks moves with two **whole-game**, probability-weighted strategy terms (it also runs the `followup_score_profile` look-ahead tie-break on the top group):
  - **Defense:** `gift_risk` finds any *open 5-line the opponent could actually complete* — i.e. a copy of the one completing tile is unaccounted-for (`completer_copies_available > 0`) **and** the open-end cell is a legal placement for it (perpendicular neighbors don't poison it, checked via `is_legal_move`) — and returns the completer tile plus how many copies are unaccounted-for. `apply_strategy_adjustments` scales `QWIRKLE_GIFT_PENALTY` (12) by `probability_in_opponent_hand`, the exact hypergeometric odds that at least one of those unseen copies is in the opponent's hand (vs. still in the bag) given the current unseen-tile pool. 2 unseen copies is treated as riskier than 1, and the risk shrinks as the unseen pool includes more tiles the opponent *isn't* holding. `gifts_opponent_qwirkle` is a thin boolean wrapper around `gift_risk` kept for callers that only need the tile.
  - **Offense:** `build_bonus` scales up to `QWIRKLE_BUILD_BAND` (3) for keeping a strong partial-Qwirkle set in hand (`partial_qwirkle_missing_tiles`, gated on `MIN_BUILD_SET`), multiplied by a confidence factor — the average, across the set's still-missing tiles, of `completer_copies_available / COPIES_PER_TILE`. A set whose missing tiles are still plentiful keeps close to the full band; one whose missing tiles are already all visible elsewhere earns close to nothing.
  - Both terms are bounded at their flat knobs (max penalty 12, max bonus 3), so a real gift can never be rescued by build value even at full confidence. Displayed scores stay raw; `strategy_note` annotates *why* a move moved in the ranking, including the computed gift-risk percentage.
