#!/usr/bin/env python3
"""Sync board.svg comments and hand labels with values in qwirkle-solver.py

Usage: python sync_board.py

This script extracts `game_state` and the ordered list passed to `Counter([...])`
from `qwirkle-solver.py` and updates `board.svg`'s coordinate comments and the
hand labels under `My Tiles` so they match the current data in the Python file.
"""
import re
import ast
...
def extract_game_state_and_hand(py_text: str):
    ...
    if m:
        start = m.start()
        open_idx = py_text.find("{", start)
        close_idx = _find_matching(py_text, open_idx, "{", "}")
        dict_src = py_text[open_idx: close_idx + 1]
        # We need Tile(...) calls to be available in eval namespace
        ns = {"Tile": Tile}
        game_state = eval(dict_src, ns)

    # Find my_tiles = Counter([ ... ]) and extract the list literal so we preserve order
...
def sync_svg(game_state: dict, hand_list: list):
    ...
    # Replace board coordinate comments like <!-- (x,y) ... -->
    def board_comment_repl(m):
        gx = int(m.group(1))
        gy = int(m.group(2))
        tile = game_state.get((gx, gy))
        if tile is None:
            # keep original comment if tile not present
            return m.group(0)
    ...
    # Update hand labels: find <text ... y=\"900\">...</text> under My Tiles
    hand_texts = re.findall(r"(<text\s+[^>]*y=\"900\"[^>]*>)([^<]*)(</text>)", s)
    if hand_texts:
        # Build replacement strings sequentially from hand_list
        labels = [f"{t.color} {t.shape}" for t in hand_list]
        if len(labels) < len(hand_texts):
            s = SVG.read_text(encoding="utf-8")

            # Build a color map used in the SVG
            color_map = {
                "red": "#e02020",
                "blue": "#1a6edb",
                "green": "#22aa33",
            ...
            # Generate board tiles section in a predictable order (sort by x then y desc to match y-flip)
            board_tiles = []
